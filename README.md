...
- **多交易所统一视图**：集中管理账户、订单、持仓和行情，降低多平台切换成本。
- **策略驱动交易能力**：内置 Bot / 策略运行时，支持自动化执行、回测与模拟验证。
- **实时数据与可观测性**：通过 GraphQL 与可选 WebSocket 提供实时交互能力，并支持健康检查与指标暴露。
- **单体部署友好**：后端服务与前端界面协同设计，支持本地开发、Docker 一键部署和手动构建。
- **可扩展工程体系**：围绕 `sqlc`、`gqlgen`、`goverter` 建立生成链路，便于持续演进业务模型与接口。

## 软件使用限制

- 合约账户仅支持**单币种保证金模式**；不支持多资产、跨币种或组合保证金模式。
- 持仓模式支持**单向持仓**与**双向持仓**；保证金类型仅支持全仓，**不支持逐仓模式**。
- 现货交易手续费**不支持使用计价币支付**；请在交易所侧关闭或避免使用计价币支付现货手续费的配置。
