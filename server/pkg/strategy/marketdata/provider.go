...
// MarketProvider 统一的市场数据提供器接口
type MarketProvider interface {
	GetMarkets(ctx context.Context, ex ctypes.Exchange) ([]*ctypes.Market, error)
	GetMarket(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (*ctypes.Market, error)

	// 价格数据
	GetLastPrice(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (decimal.Decimal, error)
	GetBookPrice(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (decimal.Decimal, decimal.Decimal, error)
	GetMarkPrice(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (decimal.Decimal, error)
	GetIndexPrice(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (decimal.Decimal, error)

	GetPriceInBaseCurrency(ctx context.Context, asset string, quote string) (decimal.Decimal, error)

	// 行情数据
	GetTicker(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (*ctypes.Ticker, error)
	GetTrades(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol, limit int) ([]*ctypes.Trade, error)
	GetDepth(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol, limit int) (*ctypes.OrderBook, error)
	GetKlines(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol, interval ctypes.Interval, limit int) ([]*ctypes.Kline, error)
	GetHisKlines(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol, interval ctypes.Interval, startTs *time.Time, endTs *time.Time, limit *int) ([]*ctypes.Kline, error)

	// 期货相关数据
	GetFundingRate(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (*ctypes.FundingRate, error)
	GetHisFundingRates(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol, startTs *time.Time, endTs *time.Time, limit *int) ([]*ctypes.FundingRate, error)
	GetOpenInterest(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (decimal.Decimal, error)
...
func NewGlobalMarketProviderWithBus(baseExchange ctypes.Exchange, baseCurrency string, bus mb.Bus) *GlobalMarketProvider {
	provider := NewGlobalMarketProvider(baseExchange, baseCurrency)

	// 市场数据提供器应该最先处理事件（更新缓存供后续阶段使用）
	bus.Subscribe(func(ctx context.Context, event stypes.Signal) error {
		return provider.OnEvent(ctx, event)
	}, int(mb.StageMarketData), mb.NewMarketSignalFilter())
...
func (p *GlobalMarketProvider) Close() {
	p.mu.Lock()
	defer p.mu.Unlock()
	if p.stopped.Load() {
		return
...
func (p *GlobalMarketProvider) OnEvent(ctx context.Context, ev stypes.Signal) error {
	...
	case p.ch <- ev:
	default:
		log.Warn().Str("type", ev.GetType().String()).Msg("market data provider channel full, dropping event")
	}
	p.mu.Unlock()
	return nil
}

func (p *GlobalMarketProvider) OnKlineEvent(ev stypes.Signal) error {
	kline := ev.(*stypes.KlineSignal)

	if ev.GetTimestamp().Before(time.Now().Add(-3 * time.Second)) {
		return nil
	}
	...
	ddl := kline.Ts.Add(CacheTTLKlines)
	p.cache.SwapByFn(key, func(old any, exp time.Time, ok bool) (any, time.Time, bool) {
		...
		if klines, ok := old.([]*ctypes.Kline); ok && len(klines) > 0 {
			...
			if last.OpenTs.After(openTs) {
				return klines, exp, false
			}
			// 当前 K 线
			if last.OpenTs.Equal(openTs) {
				klines[len(klines)-1] = &ctypes.Kline{
					Exchange: *kline.Exchange,
					Symbol:   *kline.Symbol,
			...
			if last.OpenTs.Add(interval).Equal(openTs) {
				...
				last.CloseTs = openTs
				// 创建新 K 线
				klines = append(klines, &ctypes.Kline{
					Exchange: *kline.Exchange,
					Symbol:   *kline.Symbol,
					Interval: kline.Interval,
					Open:     kline.Open,
...
func (p *GlobalMarketProvider) OnTradeEvent(ev stypes.Signal) error {
	trade := ev.(*stypes.TradeSignal)

	if ev.GetTimestamp().Before(time.Now().Add(-5 * time.Second)) {
		return nil
	}
	...
	p.cache.SwapByFn(key, func(old any, exp time.Time, ok bool) (any, time.Time, bool) {
		...
		if trades, ok := old.([]*ctypes.Trade); ok && len(trades) > 0 {
			...
			trades = append(trades, &ctypes.Trade{
...
func (p *GlobalMarketProvider) OnDepthEvent(ev stypes.Signal) error {
	if ev.GetTimestamp().Before(time.Now().Add(-5 * time.Second)) {
		return nil
	}

	depth := ev.(*stypes.DepthSignal)
	...
	p.cache.SwapByFn(key, func(old any, exp time.Time, ok bool) (any, time.Time, bool) {
		...
		if needRefresh {
			snapshot, err := proxy.GetOrderBook(context.Background(), *depth.Exchange, *depth.Symbol, DepthCacheSize)
			if err != nil {
...
func (p *GlobalMarketProvider) OnTickerEvent(ev stypes.Signal) error {
	if ev.GetTimestamp().Before(time.Now().Add(-5 * time.Second)) {
		return nil
	}

	signal := ev.(*stypes.TickerSignal)
...
func (p *GlobalMarketProvider) OnMarkPriceEvent(ev stypes.Signal) error {
	if ev.GetTimestamp().Before(time.Now().Add(-3 * time.Second)) {
		return nil
	}

	signal := ev.(*stypes.MarkPriceSignal)
...
// GetMarket 获取市场
func (p *GlobalMarketProvider) GetMarket(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (*ctypes.Market, error) {
...
// GetLastPrice 获取最新价格
func (p *GlobalMarketProvider) GetLastPrice(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (decimal.Decimal, error) {
...
// GetBookPrice 获取盘口价格
func (p *GlobalMarketProvider) GetBookPrice(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (decimal.Decimal, decimal.Decimal, error) {
...
// GetMarkPrice 获取标记价格
func (p *GlobalMarketProvider) GetMarkPrice(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (decimal.Decimal, error) {
...
func (p *GlobalMarketProvider) GetIndexPrice(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (decimal.Decimal, error) {
...
func (p *GlobalMarketProvider) GetFundingRate(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (*ctypes.FundingRate, error) {
...
func (p *GlobalMarketProvider) GetHisFundingRates(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol, startTs *time.Time, endTs *time.Time, limit *int) ([]*ctypes.FundingRate, error) {
...
func (p *GlobalMarketProvider) GetOpenInterest(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (decimal.Decimal, error) {
...
// GetTicker 获取ticker
func (p *GlobalMarketProvider) GetTicker(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol) (*ctypes.Ticker, error) {
...
func (p *GlobalMarketProvider) GetKlines(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol, interval ctypes.Interval, limit int) ([]*ctypes.Kline, error) {
...
func (p *GlobalMarketProvider) GetHisKlines(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol, interval ctypes.Interval, startTs *time.Time, endTs *time.Time, limit *int) ([]*ctypes.Kline, error) {
...
func (p *GlobalMarketProvider) GetDepth(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol, limit int) (*ctypes.OrderBook, error) {
...
func (p *GlobalMarketProvider) GetTrades(ctx context.Context, ex ctypes.Exchange, symbol ctypes.Symbol, limit int) ([]*ctypes.Trade, error) {
...
func (p *GlobalMarketProvider) GetPriceInBaseCurrency(ctx context.Context, asset string, quote string) (decimal.Decimal, error) {
