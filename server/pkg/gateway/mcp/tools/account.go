...
// 工具名 llt_* 与 GraphQL 根字段映射见各 Tool.Description。
...
func RegisterAccountTools(s *mcp.Server, rsv *resolver.Resolver) {
	mcp.AddTool(s, &mcp.Tool{Name: "llt_accounts", Description: "Query Accounts：分页查询交易账户（GraphQL Accounts）"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_equitys", Description: "Query Equitys：账户权益时间序列（GraphQL Equitys，range: 1d/7d/30d）"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_account_leverage", Description: "Query Leverage：查询账户在某 symbol 上的杠杆（GraphQL Leverage）"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_account_metrics", Description: "Query AccountMetrics：账户绩效指标（GraphQL AccountMetrics）"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_risk_events", Description: "Query RiskEvents：风控事件列表（GraphQL RiskEvents）"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_balance", Description: "Query Balance：账户余额（GraphQL Balance）"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_positions", Description: "Query Positions：账户持仓（GraphQL Positions）"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_orders", Description: "Query Orders：账户订单分页（GraphQL Orders）"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_online_account", Description: "Mutation OnlineAccount：上线账户"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_offline_account", Description: "Mutation OfflineAccount：下线账户"},
		func(ctx context.Context, _ *mcp.CallToolRequest, in accountIDIn) (*mcp.CallToolResult, any, error) {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_refresh_account_snapshots", Description: "Mutation RefreshAccountSnapshots：刷新账户快照"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_place_order", Description: "Mutation PlaceOrder：下单（高风险）"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_cancel_order", Description: "Mutation CancelOrder：撤单"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_set_leverage", Description: "Mutation SetLeverage：设置杠杆"},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "llt_update_account_risk_config", Description: "Mutation UpdateAccountRiskConfig：更新账户风控配置"},
