...
// stubBase 实现 mdtypes.Connector，避免测试依赖 GetConnector 工厂与全局 KV。
...
type mockVirtualSubReader struct {
...
func (m *mockVirtualSubReader) GetAssets(ctx context.Context, accountID string) ([]*ctypes.Asset, error) {
	m.lastAccountID = accountID
	return m.assets, nil
}

func (m *mockVirtualSubReader) GetPositions(ctx context.Context, accountID string) ([]*ctypes.Position, error) {
	m.lastAccountID = accountID
	return m.positions, nil
}

func (m *mockVirtualSubReader) GetOpenOrders(ctx context.Context, accountID string, symbol *ctypes.Symbol) ([]*ctypes.Order, error) {
	m.lastAccountID = accountID
	return m.open, nil
}

func (m *mockVirtualSubReader) GetOrder(ctx context.Context, accountID string, symbol string, clientOrderID, exchangeOrderID string) (*ctypes.Order, error) {
	m.lastAccountID = accountID
	if m.oneErr != nil {
		return nil, m.oneErr
	}
...
func TestVirtualSubConnectorView_GetOrdersUsesSubAccount(t *testing.T) {
	base := stubBase{isPrivate: true}
	src := &mockVirtualSubReader{open: []*ctypes.Order{{OrderID: "o1"}}}
