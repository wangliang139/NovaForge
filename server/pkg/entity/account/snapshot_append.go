...
func (e *Entity) appendAccountAssetSnapshotIfChanged(
	ctx context.Context,
	...
	if err != nil {
		logger.Ctx(ctx).Err(err).
			...
			Msg("insert account_asset_snapshot")
	}
}

func (e *Entity) appendAccountAssetSnapshotFromUpsertRow(ctx context.Context, row *assets.UpsertAssetRow) {
	if row == nil {
		return
	}
	...
	e.appendAccountAssetSnapshotIfChanged(ctx, row.AccountID, row.Exchange, row.WalletType, row.Asset,
		prevT, prevF, newT, newF, row.LastUpdatedTs)
}

func (e *Entity) appendAccountPositionSnapshotFromUpsertRow(ctx context.Context, row *positions.UpsertPositionRow) {
	...
	if err != nil {
		logger.Ctx(ctx).Err(err).
			...
			Msg("insert account_position_snapshot")
	}
}
