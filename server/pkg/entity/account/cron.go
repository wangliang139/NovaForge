...
func (e *Entity) RefreshSingleAccountSnapshots(ctx context.Context, accountId string) error {
	...
	return e.WithSortedAccountWrites(ctx, lockIDs, func(ctx context.Context) error {
		...
		case types.AccountTypeVirtualSub:
			...
			if err := e.publishVsAcctSnapshotsFromDB(ctx, acct.ID, acct.Exchange); err != nil {
				return err
			}
		default:
			return fmt.Errorf("account type not supported")
		...
		// 4. 每次刷新后强制追加一条资金/仓位历史快照（即使值未变化）
		if err := e.appendSnapshotRecordsAfterRefresh(ctx, acct.ID, acct.Exchange); err != nil {
			return fmt.Errorf("append snapshot records after refresh failed: %w", err)
		}
	...
	case types.AccountTypeVirtualSub:
		...
		if len(parentOrders) > 0 {
			for _, ord := range parentOrders {
				...
				if d, ok := scaled[accountID]; ok {
					ordersList = append(ordersList, &d)
				}
			}
		}
	default:
